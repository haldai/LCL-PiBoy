...
local function notify_vol(n)
   ...
   elseif n == 0 then -- toggle
      if stat == "on" then
         naughty.notify({ text = "Audio: ON", timeout = 0.5 })
      else
         naughty.notify({ text = "Audio: OFF", timeout = 0.5 })
      end
   end
end
...
local function notify_bright(n)
   ...
   if n == 1 then -- up
      naughty.notify({ text = "Brightness +:" .. bri .. "%", timeout = 0.5 })
   elseif n == -1 then -- down
      naughty.notify({ text = "Brightness -:" .. bri .. "%", timeout = 0.5 })
   end
end
...
-- {{{ Menu
local myawesomemenu = {
   { " Help      ", function() return false, hotkeys_popup.show_help end, icon_path .. "devices/keyboard.png" },
   { " Settings  ", string.format("%s -e %s %s", terminal, editor, awesome.conffile), icon_path .. "categories/system-settings.png" },
   { " Reload    ", awesome.restart, icon_path .. "actions/system-reboot.png" },
   { " Exit (TTY)", function() awesome.quit() end, icon_path .. "actions/system-log-out.png" }
}

awful.util.mymainmenu = awful.menu({
      items = {
         { " Terminal", function () awful.spawn(terminal) end, icon_path .. "apps/utilities-terminal.png" },
         { " Editor  ", function () awful.spawn("emacs") end, icon_path .. "apps/emacs.png" },
         { " Firefox ", function () awful.spawn("firefox") end, icon_path .. "apps/firefox.png" },
         { " Files   ", function () awful.spawn("pcmanfm") end, icon_path .. "apps/file-manager.png" },
         { " Monitor ", function () awful.spawn("st -e htop") end, icon_path .. "apps/utilities-system-monitor.png" },
         { " Audio   ", function () awful.spawn("pavucontrol") end, icon_path .. "apps/sound.png" },
         { " Awesome ", myawesomemenu, beautiful.awesome_icon },
         { " Screens ", function () awful.spawn("arandr") end, icon_path .. "devices/system.png" },
         { " Poweroff", "poweroff", icon_path .. "actions/system-shutdown.png" }
      }
})
...
globalkeys = gears.table.join(
   ...
   awful.key({ modkey }, "-", function ()
         scratch.drop(terminal, "center", "center", 400, 300) end,
   ...
   awful.key({ modkey }, "F3", function () awful.spawn("firefox") end,
      {description = "launch firefox", group = "launcher"}),
   awful.key({ modkey, "Ctrl" }, "F3", function () awful.spawn("qutebrowser") end,
      {description = "launch qutebrowser", group = "launcher"}),
   awful.key({ modkey, "Shift" }, "F3", function () awful.spawn("firefox") end,
      {description = "launch firefox with user data dir", group = "launcher"}),
   ...
   awful.key({ modkey }, "F6", function () awful.spawn("dbus-send --type=method_call --dest=org.onboard.Onboard /org/onboard/Onboard/Keyboard org.onboard.Onboard.Keyboard.ToggleVisible") end,
      {description = "launch on-screen keyboard", group = "launcher"}),
...
awful.rules.rules = {
   ...
   {
     rule_any = {
       class = {"Retroarch", "retroarch", "Moonlight", "moonlight", "SteamLink", "shell"},
       name = {"Retroarch", "retroarch", "Moonlight", "moonlight", "SteamLink", "shell"},
     },
