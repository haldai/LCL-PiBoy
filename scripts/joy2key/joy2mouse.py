...
logging.basicConfig(level=logging.INFO,
                    format=u"%(asctime)s %(levelname)-6s %(message)s")
...
def ra_event_map(input_str: str) -> str:
    ...
    """
    ra_event_map = {
        'up': ['r_y_minus'],  # 'l_y_minus'
        'down': ['r_y_plus'],  # 'l_y_plus'
        'left': ['r_x_minus'],  # 'l_x_minus'
        'right': ['r_x_plus'],  # 'l_x_plus'
        'pageup': 'l',
        'pagedown': 'r',
        'lclick': 'l2',
        'rclick': 'r2'
    ...
    input_norm = input_str.replace('input_', '').replace(
        '_axis', '').replace('_btn', '')
    ...
    try:
        if key.endswith('btn'):
            if value.startswith('h'):
                ...
                input_index, input_value = int(
                    hat_value[0]), JS_HAT_VALUES[hat_value[1]]
            else:
                input_type = 'button'
...
def get_all_ra_config(def_buttons: list) -> list:
    ...
    generic_dev.add_mappings(
        {},  # no axis
        # D-Pad as 'hat0'
        {0: [(1, 'up'), (8, 'left'), (4, 'down'), (2, 'right')]},
        {0: 'b', 1: 'a', 3: 'y', 4: 'x'}  # 4 buttons
    )
    ra_config_list.append(generic_dev)
    js_cfg_dir = CONFIG_DIR + 'retroarch/autoconfig/'
            ...
            try:
                ...
                for i in conf_vals:
                    if i.startswith('input') and (i.endswith('btn') or i.endswith('axis')):
                        input_type, input_index, input_value = ra_input_parse(
                            i, conf_vals[i].strip('"'))
                        ...
                        if input_type == 'button':
                            ...
                            hats.setdefault(input_index, []).append(
                                (input_value, event_name))
                        elif input_type == 'axis':
                            axis.setdefault(input_index, []).append(
                                (input_value, event_name))
                        else:
                            continue
...
def ra_btn_swap_config():
    ...
    """
    config = ConfigParser(delimiters="=", strict=False, interpolation=None)
    with open(CONFIG_DIR + 'retroarch/retroarch.cfg', 'r') as cfg_file:
        config.read_string('[device]\n' + cfg_file.read())
        try:
